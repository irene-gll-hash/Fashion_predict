...
from sources.zara.loader import find_zara_input_file, load_zara_products
from sources.zara.normalizer import build_enriched_zara_product, get_zara_image_urls, normalize_zara_records
...
def run_zara(date: str, input_file: Path | None, limit: int | None, max_images_per_product: int, skip_gemini: bool, sleep_seconds: float, max_retries: int) -> None:
    run_dir = DATA_DIR / "zara" / date
    run_dir.mkdir(parents=True, exist_ok=True)
    input_path = input_file or find_zara_input_file(DATA_DIR, date)
    normalized_file = run_dir / "normalized_zara.json"
    gemini_file = run_dir / "gemini_zara_analysis.json"
    enriched_file = run_dir / "enriched_zara.json"
    images_dir = run_dir / "images"
    raw_products = load_zara_products(input_path)
    if limit is not None:
        raw_products = raw_products[:limit]
    normalized_products = normalize_zara_records(raw_products)
    save_json(normalized_file, normalized_products)
    gemini_results = _load_existing_gemini_results(gemini_file)
    analyzer = None if skip_gemini else ProductGeminiAnalyzer(sleep_seconds=sleep_seconds, max_retries=max_retries)
    for index, product in enumerate(normalized_products, start=1):
        product_id = product["product_id"]
        print(f"[{index}/{len(normalized_products)}] {product_id}")
        image_urls = get_zara_image_urls(product)
        product_images_dir = images_dir / product_id
        image_paths = download_images(image_urls, product_images_dir, max_images=max_images_per_product)
        if skip_gemini:
            continue
    ...
    for product in normalized_products:
        ...
        enriched_products.append(build_enriched_zara_product(product, gemini_result))
...
def main() -> None:
    args = parse_args()
    run_zara(date=args.date, input_file=args.input_file, limit=args.limit, max_images_per_product=args.max_images_per_product, skip_gemini=args.skip_gemini, sleep_seconds=args.sleep_seconds, max_retries=args.max_retries)
